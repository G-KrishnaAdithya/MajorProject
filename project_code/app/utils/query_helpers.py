...
class QueryHelper:
    ...
    def setup_indexes():
        """
        Create indexes for various collections to optimize query performance.

        This method sets up indexes for common query patterns across different collections.
        It's called once when the module is loaded to ensure indexes exist.
        """
        try:
            # Chats collection:
            QueryHelper.db["chats"].create_index(
                [("user_id", 1), ("status", 1), ("created_on", -1)]
            )
            QueryHelper.db["chats"].create_index([("user_id", 1)])
            QueryHelper.db["chats"].create_index([("user_id", 1), ("chat_type", 1)])
            QueryHelper.db["chats"].create_index([("is_deleted", 1)])

            # Chat messages collection:
            QueryHelper.db["chat_messages"].create_index(
                [("chat_id", 1), ("created_on", 1)]
            )
            QueryHelper.db["chat_messages"].create_index([("_id", 1), ("chat_id", 1)])
            QueryHelper.db["chat_messages"].create_index([("chat_id", 1)])
            QueryHelper.db["chat_messages"].create_index([("user_id", 1)])
            QueryHelper.db["chat_messages"].create_index([("message_type", 1)])

            # Tip of the day collection:
            QueryHelper.db["tip_of_the_day"].create_index("created_on")

            # Posts collection - add text indexes for better search
            QueryHelper.db["posts"].create_index([("status", 1), ("softDeleted", 1)])
            QueryHelper.db["posts"].create_index([("userId", 1)])
            QueryHelper.db["posts"].create_index([("tags", 1)])
            QueryHelper.db["posts"].create_index([("topic", 1)])

            # Collections collection - add text indexes for better search
            QueryHelper.db["collections"].create_index(
                [("name", "text"), ("description", "text"), ("topic", "text")]
            )
            QueryHelper.db["collections"].create_index([("status", 1)])

            # Users collection
            QueryHelper.db["users"].create_index([("status", 1)])
            QueryHelper.db["users"].create_index([("roles", 1)])

            # Credits collection
            QueryHelper.db["credits"].create_index([("userId", 1), ("status", 1)])
            QueryHelper.db["credits"].create_index([("expiryDate", 1)])
            QueryHelper.db["credits"].create_index([("remainingCredits", 1)])

            # Credit usage history collection
            QueryHelper.db["credit_usage_history"].create_index([("userId", 1)])
            QueryHelper.db["credit_usage_history"].create_index([("chat_id", 1)])
            QueryHelper.db["credit_usage_history"].create_index([("usageType", 1)])
            QueryHelper.db["credit_usage_history"].create_index([("created_on", 1)])

        except Exception as e:
            # Raising an exception here means that any failure in index creation
            # will be immediately visible when the method is called.
            raise Exception(f"Error in creating indexes: {e}")


# Call setup_indexes once when the module is loaded.
QueryHelper.setup_indexes()
